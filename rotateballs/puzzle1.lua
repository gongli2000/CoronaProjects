...
function new()
    ...
    local width=150
    local height = 150
    local tapRect = display.newRect(left,top,width,height)
    tapRect:setFillColor(255,255,255,0)
    tapRect:setStrokeColor(255,0,0)
    tapRect.strokeWidth = 6

    local xmin=left
    local xmax=xmin+width
    local ymin=top
    local ymax=ymin+height
    print (tapRect.contentBounds [1])
    local function tap(event)
        if (infotext ~= nil) then
            infotext.text = ""
        end
        if (event.x > xmin and event.x < xmax and event.y > ymin and event.y < ymax) then
            --local ball = display.newCircle( event.x, event.y, ballRadius )
            local ball = display.newImage("pig.png")
            ball.x = event.x
            ball.y = event.y
            ball.xScale = .25
            ball.yScale = .25
